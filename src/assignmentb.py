...
# ## 7. Instructions for Full Implementation

print("""
INSTRUCTIONS FOR FULL IMPLEMENTATION:
====================================

To use this code with the actual Apache SpamAssassin dataset:

1. Download the SpamAssassin public corpus from:
   https://spamassassin.apache.org/old/publiccorpus/

2. Extract the files to directories:
   - ham/: containing legitimate emails
   - spam/: containing spam emails

3. Replace the sample_emails list with:
   
   ham_emails = load_emails_from_directory('path/to/ham/', 0)
   spam_emails = load_emails_from_directory('path/to/spam/', 1)
   all_emails = ham_emails + spam_emails

4. The rest of the pipeline will work with the full dataset.

DESIGN DECISIONS EXPLAINED:
==========================

1. **Preprocessing Pipeline**: Modular design with configurable hyperparameters
   - Strip headers: Removes email metadata that might not be relevant
   - Lowercase: Normalizes text for consistent feature extraction
   - URL/Number replacement: Reduces vocabulary size and generalizes patterns
   - Punctuation removal: Focuses on word content

2. **Feature Extraction**: Bag of Words (CountVectorizer)
   - Simple and effective for text classification
   - Captures word presence/absence or frequency
   - Limited to top 1000 features to prevent overfitting

3. **Multiple Classifiers**: Compared three different approaches
   - Naive Bayes: Fast, works well with text data
   - Logistic Regression: Linear model with good interpretability
   - SVM: Can capture complex patterns with linear kernel

4. **Evaluation Metrics**: Focus on both precision and recall
   - High precision: Few false positives (legitimate emails marked as spam)
   - High recall: Few false negatives (spam emails missed)
   - Both are important for a spam filter

This implementation provides a solid foundation for spam classification
that can be easily extended and improved with the full dataset.
""")